...
`research/korean-equity-rwa/sources/user/`는 팀원이 제공한 변경 금지 원본 영역이다. 아래 체크섬은 2026-08-28 재확인한 SHA-256이며, 이 문서는 원본의 위치와 설계 반영 여부를 감사할 수 있게 한다. 팀 문서는 유용한 선행조사이지만 공식 법령·감독자료·표준과 충돌할 경우 후자를 우선한다.
...
| U004 | `research/korean-equity-rwa/sources/user/경서님/W2_vasp_지급결제_투자자식별모델구조화.pdf` | `17450b72d6ba70facd7c860204678bf40f0c87a5d8f740bc157f965b4838e44b` | 지급결제와 VASP 쟁점 참고. USDC 자금 흐름은 모의하되 실제 CCIP와 브리지 연동은 제외 |
...
| U012 | `research/korean-equity-rwa/sources/user/휘서님/W3_한국주식RWA_로컬노드_데이터연동_아키텍처.pdf` | `929ff39dc31789d00daefdf4f51298fae476cf4b4f67e712eda80d891bec059f` | 어댑터 아이디어 참고. USDC 자금 흐름은 화면으로 모의하며 퍼블릭 체인의 직접 KRX 결제 구조는 제외 |
...
| U015 | `research/korean-equity-rwa/sources/user/1b30ec6e-7c96-4323-ab23-cfdfd85071b7_주식_RWA를_통한_외국인_투자자의_한국_주식_접근성_개선.pdf` | `74748c2c19c35769b8a899bc6861f8ba15a2317c9fd510fd6bfbf2764f6ff95b` | 발행인관리계좌부와 고객관리계좌부의 총량관리, 자기계좌부와 고객계좌부의 법적 권리기록, 외국인 통합계좌의 해외 최종투자자 장부 및 2027년 법정 분산원장 계좌부를 연결한 설계 기준. 법률 서술은 전자증권법, 상법과 금융위 원문으로 재검증 |
| U016 | `research/korean-equity-rwa/sources/user/final_candidate_revised_3.md` | `99d3e6724ad8647aa020bc5ab59a44b3c7adbc7f102276e7471cf0c6aee9b4dc` | 팀원의 마스터 검토 의견. 사실 근거로 인용하지 않고 배당을 현재 토큰 잔액이 아닌 기준일 권리 스냅샷으로 판단해야 한다는 지적만 선별 반영. 상세 결제 대기 상태, 래퍼, 수량 불변식과 사용자가 제외한 절의 제안은 PRD 범위 또는 미채택으로 분류 |

## 주요 충돌 판정

1. **이중 원장 대 단일 원장**: 자산 OP Stack·현금 Besu·CCIP 연결안보다, PoC에서 DvP 불변식을 실제 검증할 수 있는 단일 허가형 원장을 채택한다.
2. **전자등록주식 대 수탁 권리**: 1단계는 법적 주식 자체가 아니라 인가 해외 증권사 파트너의 고객계좌에 기록되는 수익적 또는 수탁상 권리다. 2단계에서만 전자등록기관, 계좌관리기관 또는 등록된 발행인계좌관리기관이 참여하는 법적 증권 계좌부를 전제한다.
3. **체결 직후 발행과 T+2**: 고객자금과 KRX 체결을 확인하면 해외 증권사가 고객 권리와 토큰을 발행한다. 국내 한국주식은 T+2까지 결제 대기로 구분하며 권리를 먼저 확정한 기관이 결제차이 위험을 부담한다.
...
7. **Dinari 발행시점**: 공식 제품문서는 브로커 주문 체결 완료 통지 후 발행을 설명한다. 본 PoC도 체결분을 즉시 발행하되 국내 결제 대기 수량과 인가 해외 증권사의 미결제 한도를 별도로 추적한다. Dinari와 Alpaca의 비공개 손실배분을 한국형에 존재한다고 가정하지 않는다.
