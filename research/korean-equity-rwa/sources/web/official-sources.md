...
검증 기준일은 2026-08-25이다. `A`는 법령·감독기관·공식 표준·직접 감사 가능한 1차 자료, `B`는 공식 기관의 설명 또는 기업 약관이다. 아래 요약은 원문의 대체물이 아니며 실제 사업화 시 최신 원문과 법률의견을 다시 확인해야 한다.
...
| S027 | B | [Dinari Docs, US Customers](https://docs.dinari.com/docs/us) | 2026-06-22 표시 가이드상 미국 고객은 완전공개 계좌를 사용한다. 고객계좌의 ACH 사전입금 또는 즉시자금조달, Circle을 통한 USDC→USD→브로커리지 계좌 흐름과 파트너 자금계정의 USD 송금 경로를 설명한다. 고객확인, 공시와 전자 동의, 민원 전달, 전용 지갑, 비이전 토큰과 별도 증권화면도 요구 | 일반 고객 대상 계좌와 보호통제, 법정통화 및 USDC 자금경로를 비교하고 Circle 사용사실을 확인 |
...
- S027은 Dinari의 회사 가이드이며 SEC·FINRA가 미국 서비스의 모든 법적 표현에 동의했다는 근거가 아니다. 2026년 8월 자기보관 지갑 발표와 함께 읽어야 하며 자기보관이 자유로운 이전이나 브로커 고객계좌의 소멸을 뜻하지 않는다. 파트너 자금계정의 USD 송금 흐름을 일반 개인의 직접 USD 송금과 같은 것으로 확대하지 않는다.
