...
Dinari가 2026년 SEC Crypto Task Force에 제출한 미국용 제안은 국제 dShares 설명과 구분한다. 해당 제안은 브로커의 오프체인 장부를 권위기록으로 유지하고 토큰을 이전불가·독립권리 없는 보조기록으로 설명한다. 이 제출은 회사의 확인요청이며 SEC 승인이나 무조치의견이 아니다. 따라서 벤더명이나 토큰형식만으로 법적 권리모델을 추정하지 않고 상품별 `legalForm`, 권위장부와 이전효과를 명시해야 한다.

본 PoC는 Dinari를 공급자나 필수 연동기관으로 채택하지 않는다. 비교에서 추출하는 결정은 다음 세 가지뿐이다.
