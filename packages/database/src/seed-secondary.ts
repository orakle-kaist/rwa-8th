  ...
  try {
    ...
    await client.query(
      `INSERT INTO local_simulation_instruments
        (security_id,display_name,token_symbol,synthetic_deployment_key,reference_security_id,
         reference_limit_krw,primary_enabled,secondary_enabled)
       ...
       ON CONFLICT (security_id) DO UPDATE SET display_name=EXCLUDED.display_name,
         ...
         reference_limit_krw=EXCLUDED.reference_limit_krw,secondary_enabled=true`,
      [
        LOCAL_SECONDARY_SECURITY_ID,
        LOCAL_SECONDARY_NAME,
        LOCAL_SECONDARY_SYMBOL,
    ...
    await client.query(
      `INSERT INTO market_maker_positions
        (principal_id,security_id,starting_settled_quantity,pending_quantity,net_position,updated_at)
       VALUES ($1,$2,$3,$4,0,$5)
    ...
    await client.query(
      `INSERT INTO secondary_market_state
        (security_id,reference_usd_minor,information_effective_at,usdc_usd_ppm,half_spread_bps,
         security_loss_bps,portfolio_loss_bps,updated_at)
       VALUES ($1,$2,$3,1000000,50,0,0,$3)
      ...
      [LOCAL_SECONDARY_SECURITY_ID, LOCAL_SECONDARY_REFERENCE_USD_MINOR.toString(), now],
    );
