...
> 현재 상태: **마스터 목표·분류체계 재승인 대기**
>
> 다음 행동: [마스터 설계](docs/01-master/MASTER.md)의 첫 질문과 접근경로 및 권리모델 분리를 다시 승인한다. 기존 10단계 구현 증거는 보존하지만 재승인 전에는 10단계 최종 승인과 11단계를 진행하지 않는다.
>
...
| 1. 마스터 확정 | 목표·분류체계 재승인 대기 | [마스터 설계](docs/01-master/MASTER.md) | 프로젝트 목적, 계좌 경로, 토큰화 권리모델과 기관 역할을 이해하고 이번 개정안을 승인할 때 |
| 2. PoC 정의 | 기존 승인 유지, 상위 마스터 재승인 대기 | [목표와 성공 기준](docs/02-poc-definition/POC_GOALS.md), [시험 데이터](docs/02-poc-definition/POC_TEST_DATA.md) | 구현 범위, 불변식, 대표 종목과 합성 통제값을 확인할 때 |
