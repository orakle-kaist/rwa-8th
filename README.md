...
> 현재 상태: **8단계 스마트컨트랙트 설계 검토 대기**
>
> 다음 행동: 제한형 권리토큰의 계약 구조, 인터페이스, 권한과 불변식을 함께 검토하고 승인한다. 승인 전에는 9단계 테스트 설계를 시작하지 않는다.
>
...
9단계 이후 폴더는 해당 단계가 시작될 때 만든다. 빈 폴더나 내용이 정해지지 않은 문서를 미리 만들지 않는다.
...
- 8단계 네 문서와 계약 명세는 팀 내부 검토안이다. 승인 전에는 9단계나 구현 기준으로 사용하지 않는다.
