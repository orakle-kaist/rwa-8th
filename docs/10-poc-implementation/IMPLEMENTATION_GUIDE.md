...
두 번째 기능의 구현범위와 시험 결과는 [제한형 토큰 기반 구현 증거](TOKEN_FOUNDATION_EVIDENCE.md), 세 번째 기능은 [고객·상품·투자자 보호 구현 증거](ELIGIBILITY_AND_PROTECTION_EVIDENCE.md)에 정리한다.

## 1. 현재 구현 범위

실행 기반과 제한형 토큰 기반 위에 세 번째 기능인 고객·상품·투자자 보호 흐름을 구현했다.
...
현재까지 고객이 주문 전에 갖춰야 할 판정, 동의와 전용 지갑 및 상품 후보 조회까지 구현했다. 체결·배분과 권리기입 증거를 결합하는 발행 컨트롤러, USD·USDC 정산, 환매 지급청구와 독립승인 기업행동은 이후 생애주기 기능에서 구현한다.
