...
| 경로 | 사용자 | 결과 |
|---|---|---|
| `GET /session` | 전체 | 합성 사용자, 역할과 기관 |
| `GET /products` | 전체 | 상품목록, 기능별 가능상태와 기준정보 버전 |
...
- [ ] 모든 상태변경 명령이 `202`와 조회 가능한 업무 ID를 반환한다.
- [ ] 모든 명령에 멱등키, 상관관계 ID와 역할통제가 있다.
- [ ] 투자자, 기관과 어댑터 API가 같은 공통 schema를 참조한다.
- [ ] 결제, 수탁과 체인 완료를 동기 응답으로 가장하지 않는다.
- [ ] 불완전하거나 오래된 투영이 신규 자산이동에 사용되지 않는다.
- [ ] 오류가 책임 역할과 다음 행동을 설명한다.

승인 전에는 8단계 스마트컨트랙트 설계를 시작하지 않는다.
