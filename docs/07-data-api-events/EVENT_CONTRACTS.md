...
- [ ] 모든 이벤트에 고유 ID, 업무 연결, 원인과 순서정보가 있다.
- [ ] 중복 이벤트가 두 번째 자산효과를 만들지 않는다.
- [ ] 순번공백과 알 수 없는 주요버전이 격리된다.
- [ ] 기관 정정이 원이벤트를 덮어쓰지 않는다.
- [ ] 1차 발행, 24/7 거래, 환매와 권리업무의 완료증거가 구분된다.
- [ ] PostgreSQL 발송함만으로 재시작 뒤 흐름을 복원할 수 있다.
