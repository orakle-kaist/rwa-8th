...
import { LOCAL_REDEMPTION_TOTAL_USD_MINOR, allocateUsdClaims } from "@rwa/domain";
...
export class LocalChainSynchronizer {
  ...
  static async create(pool: Pool, environment: NodeJS.ProcessEnv) {
    const path =
      environment.LOCAL_CHAIN_MANIFEST_PATH ??
      resolve(process.cwd(), ".runtime/local-deployment.json");
    ...
    if (started.record.status === "SUBMITTED" || started.record.status === "UNCERTAIN") {
      ...
      if (knownHash) {
        const receipt = await this.publicClient
          .getTransactionReceipt({ hash: knownHash })
          .catch(() => undefined);
  ...
  private async synchronizePrimary(workflowId: string) {
    ...
    const payload = row.request_payload as {
      signedIntent?: { message?: Record<string, string>; signature?: Hex };
    };
    ...
    const scoped = (label: string, value: unknown) =>
      evidence(`${workflowId}:${label}:${String(value ?? "MISSING")}`);
    const calls: Array<[string, string, readonly unknown[]]> = [
      [
        "PRIMARY_EXECUTION_ALLOCATION",
        "confirmExecutionAllocation",
        [
          workflow,
          token,
          investor,
          BigInt(String(row.batch_filled_quantity ?? quantity)),
          quantity,
          scoped("EXECUTION", row.execution_evidence_hash),
          scoped("ALLOCATION", row.allocation_evidence_hash),
        ],
      ],
      [
        "PRIMARY_T2_RISK",
        "approveT2Risk",
        [workflow, token, investor, quantity, scoped("RISK", row.risk_evidence_hash)],
      ],
      [
        "PRIMARY_RIGHTS_APPROVAL",
        "approveRightsEntry",
        [
          workflow,
          token,
          investor,
          quantity,
          scoped("RIGHTS_APPROVAL", row.rights_approval_evidence_hash),
        ],
      ],
      [
        "PRIMARY_RIGHTS_RECORDED",
        "confirmRightsRecorded",
        [
          workflow,
          token,
          investor,
          quantity,
          scoped("RIGHTS_RECORDED", row.rights_recorded_evidence_hash),
        ],
      ],
      [
        ...
        [
          ...
          intentTuple(
            signed.message,
            ["orderId"],
            ["shareQuantity", "krwLimitPrice", "fundingAmountMinor", "nonce", "expiresAt"],
          ),
          signed.signature,
        ],
      ],
    ...
    for (const [stage, functionName, args] of calls)
      mintHash = await this.run(workflowId, stage, controller, functionName, () =>
        writeIssuanceController(this.wallet, {
          controller,
          functionName: functionName as never,
          args,
        }),
      );
    await this.pool.query("UPDATE primary_orders SET token_transaction_hash=$2 WHERE order_id=$1", [
      workflowId,
      mintHash,
    ]);
    ...
    if (row.domestic_settlement_confirmed && row.custody_quantity_confirmed) {
      const releaseCalls: Array<[string, string, readonly unknown[]]> = [
        [
          "PRIMARY_DOMESTIC_SETTLEMENT",
          "confirmDomesticSettlement",
          [workflow, token, investor, quantity, scoped("SETTLEMENT", row.settlement_evidence_hash)],
        ],
        [
          "PRIMARY_CUSTODY",
          "confirmCustodyQuantity",
          [workflow, token, investor, quantity, scoped("CUSTODY", row.custody_evidence_hash)],
        ],
      ...
      await this.pool.query(
        "UPDATE primary_orders SET release_transaction_hash=$2 WHERE order_id=$1",
        [workflowId, releaseHash],
      );
    }
  }
  ...
  private async synchronizeRedemption(workflowId: string) {
      ...
      writeRedemptionController(this.wallet, {
        ...
        args: [
          ...
          intentTuple(
            signed.message!,
            ["redemptionId"],
            ["shareQuantity", "krwLimitPrice", "nonce", "expiresAt"],
          ),
          signed.signature!,
        ],
      }),
    ...
    if (row.domestic_execution_confirmed) {
      await this.run(
        workflowId,
        "REDEMPTION_DOMESTIC_SUBMITTED",
        controller,
        "markDomesticSaleSubmitted",
        () =>
          writeRedemptionController(this.wallet, {
            controller,
            functionName: "markDomesticSaleSubmitted",
            args: [workflow, evidence(`${workflowId}:DOMESTIC_SUBMITTED`)],
          }),
      );
      await this.run(
        workflowId,
        ...
        () =>
          writeRedemptionController(this.wallet, {
            controller,
            functionName: "confirmDomesticExecution",
            args: [
              workflow,
              BigInt(String(row.allocated_quantity)),
              evidence(`${workflowId}:EXECUTION`),
            ],
          }),
      );
    }
    if (row.sale_proceeds_settled && BigInt(String(row.allocated_quantity)) > 0n) {
      const claimAmount = row.cash_claim_usd_minor
        ? BigInt(String(row.cash_claim_usd_minor))
        : await this.expectedRedemptionCashClaim(workflowId, String(row.batch_id));
      await this.run(workflowId, "REDEMPTION_T2", controller, "confirmSaleProceedsSettled", () =>
        writeRedemptionController(this.wallet, {
          controller,
          functionName: "confirmSaleProceedsSettled",
          args: [
            workflow,
            BigInt(String(row.allocated_quantity)),
            claimAmount,
            evidence(`${workflowId}:T2`),
          ],
        }),
      );
    }
    if (row.rights_terminated && row.cash_claim_usd_minor) {
      ...
      const calls: Array<[string, string, readonly unknown[]]> = [
        [
          "REDEMPTION_RIGHTS_TERMINATED",
          "confirmRightsTerminated",
          [workflow, token, investor, quantity, evidence(`${workflowId}:RIGHTS_TERMINATED`)],
        ],
        [
          "REDEMPTION_CASH_CLAIM",
          "confirmCashClaim",
          [workflow, quantity, amount, evidence(`${workflowId}:CASH_CLAIM`)],
        ],
        ["REDEMPTION_BURN_PENDING", "markBurnPending", [workflow]],
      ];
      for (const [stage, functionName, args] of calls)
        await this.run(workflowId, stage, controller, functionName, () =>
          writeRedemptionController(this.wallet, {
            controller,
            functionName: functionName as never,
            args,
    ...
    if (row.token_burned && row.cash_claim_usd_minor) {
      ...
      await this.run(
        workflowId,
        "REDEMPTION_PAYMENT_APPROVED",
        controller,
        "approveUsdPayment",
        () =>
          writeRedemptionController(this.wallet, {
            controller,
            functionName: "approveUsdPayment",
            args: [workflow, amount, evidence(`${workflowId}:PAYMENT`)],
          }),
      );
      const burnHash = await this.run(
        workflowId,
        "REDEMPTION_BURN",
        controller,
        "executeBurn",
      ...
      await this.pool.query(
        "UPDATE redemption_orders SET burn_evidence_hash=$2 WHERE redemption_id=$1",
        [workflowId, burnHash],
      );
    }
  }

  private async expectedRedemptionCashClaim(workflowId: string, batchId: string): Promise<bigint> {
    const batch = await this.pool.query<{ total_net_usd_minor: string | null }>(
      "SELECT total_net_usd_minor::text FROM redemption_batches WHERE batch_id=$1",
      [batchId],
    );
    const orders = await this.pool.query<{
      ...
      `SELECT redemption_id,allocated_quantity::text,accepted_at
         FROM redemption_orders WHERE batch_id=$1 AND allocated_quantity>0
         ORDER BY accepted_at,redemption_id`,
      [batchId],
    );
    const claims = allocateUsdClaims(
      orders.rows.map((order) => ({
        orderId: order.redemption_id,
        allocatedQuantity: BigInt(order.allocated_quantity),
        acceptedAt: order.accepted_at,
      })),
      batch.rows[0]?.total_net_usd_minor
        ? BigInt(batch.rows[0].total_net_usd_minor)
        : LOCAL_REDEMPTION_TOTAL_USD_MINOR,
    );
    const claim = claims.find((item) => item.orderId === workflowId);
    if (!claim) throw new Error("환매 T+2용 고객별 USD 지급청구를 계산할 수 없다.");
    return claim.usdAmountMinor;
  }
  ...
  private async synchronizeRecovery(workflowId: string) {
    ...
    await this.run(workflowId, "RECOVERY_NEW_WALLET_ELIGIBLE", registry, "setEligibility", () =>
      this.write(registry, eligibilityAbi, "setEligibility", [
        workflow,
        newWallet,
        true,
        BigInt(Math.floor(Date.now() / 1000) + 365 * 24 * 60 * 60),
        evidence(`${workflowId}:NEW_WALLET_ELIGIBLE`),
      ]),
    );
    const tokenAbi = parseAbi([
      "function freezeAddress(bytes16 workflowId,address account,bool frozen,bytes32 evidenceHash)",
    ]);
    await this.run(workflowId, "RECOVERY_OLD_WALLET_FROZEN", token, "freezeAddress", () =>
      this.write(token, tokenAbi, "freezeAddress", [
        workflow,
        oldWallet,
        true,
        evidence(`${workflowId}:FREEZE`),
      ]),
    );
    await this.run(
      workflowId,
      "RECOVERY_RIGHTS_APPROVAL",
      controller,
      "approveRightsRecovery",
      () =>
        writeRecoveryController(this.wallet, {
          controller,
          functionName: "approveRightsRecovery",
          args: [workflow, oldWallet, newWallet, evidence(`${workflowId}:RIGHTS`)],
        }),
    );
    await this.run(
      workflowId,
      "RECOVERY_COMPLIANCE_APPROVAL",
      controller,
      "approveComplianceRecovery",
      () =>
        writeRecoveryController(this.wallet, {
          controller,
          functionName: "approveComplianceRecovery",
          args: [workflow, oldWallet, newWallet, evidence(`${workflowId}:COMPLIANCE`)],
        }),
    );
    const hash = await this.run(
      ...
      () =>
        writeRecoveryController(this.wallet, {
          ...
          args: [workflow, token, oldWallet, newWallet],
        }),
    );
    await this.pool.query(
    ...
    await this.pool.query(
      "UPDATE workflows SET status='RECOVERY_COMPLETED',updated_at=$2 WHERE workflow_id=$1",
      [workflowId, new Date()],
    );
  }
  ...
  private async synchronizeHedge(workflowId: string) {
    ...
    const row = result.rows[0];
    if (!row?.token_address || !row.signed_intent || BigInt(String(row.filled_quantity)) === 0n)
      return;
    ...
    if (row.direction === "BUY") {
      ...
      const calls: Array<[string, string, readonly unknown[]]> = [
        [
          ...
          [
            ...
            quantity,
            evidence(`${workflowId}:EXECUTION`),
            evidence(`${workflowId}:ALLOCATION`),
          ],
        ],
        [
          "HEDGE_BUY_RISK",
          "approveT2Risk",
          [workflow, token, marketMaker, quantity, evidence(`${workflowId}:RISK`)],
        ],
        [
          "HEDGE_BUY_RIGHTS_APPROVAL",
          "approveRightsEntry",
          [workflow, token, marketMaker, quantity, evidence(`${workflowId}:RIGHTS_APPROVAL`)],
        ],
        [
          "HEDGE_BUY_RIGHTS_RECORDED",
          "confirmRightsRecorded",
          [workflow, token, marketMaker, quantity, evidence(`${workflowId}:RIGHTS_RECORDED`)],
        ],
        [
      ...
      for (const [stage, functionName, args] of calls)
        finalHash = await this.run(workflowId, stage, controller, functionName, () =>
          writeIssuanceController(this.wallet, {
            controller,
            functionName: functionName as never,
            args,
      ...
      if (row.domestic_settlement_confirmed && row.custody_quantity_confirmed) {
        for (const [stage, functionName, args] of [
          [
            "HEDGE_BUY_SETTLEMENT",
            "confirmDomesticSettlement",
            [workflow, token, marketMaker, quantity, evidence(`${workflowId}:SETTLEMENT`)],
          ],
          [
            "HEDGE_BUY_CUSTODY",
            "confirmCustodyQuantity",
            [workflow, token, marketMaker, quantity, evidence(`${workflowId}:CUSTODY`)],
          ],
      ...
      await this.pool.query(
        "UPDATE market_maker_hedges SET token_transaction_hash=$2 WHERE hedge_id=$1",
        [workflowId, finalHash],
      );
      ...
      const calls: Array<[string, string, readonly unknown[]]> = [
        ...
        [
          "HEDGE_SELL_SUBMITTED",
          "markDomesticSaleSubmitted",
          [workflow, evidence(`${workflowId}:SUBMITTED`)],
        ],
        [
          "HEDGE_SELL_EXECUTION",
          "confirmDomesticExecution",
          [workflow, quantity, evidence(`${workflowId}:EXECUTION`)],
        ],
        [
          "HEDGE_SELL_T2",
          "confirmSaleProceedsSettled",
          [workflow, quantity, amount, evidence(`${workflowId}:T2`)],
        ],
        [
          "HEDGE_SELL_RIGHTS",
          "confirmRightsTerminated",
          [workflow, token, marketMaker, quantity, evidence(`${workflowId}:RIGHTS`)],
        ],
        [
          "HEDGE_SELL_CLAIM",
          "confirmCashClaim",
          [workflow, quantity, amount, evidence(`${workflowId}:CLAIM`)],
        ],
        ["HEDGE_SELL_BURN_PENDING", "markBurnPending", [workflow]],
        [
          "HEDGE_SELL_PAYMENT",
          "approveUsdPayment",
          [workflow, amount, evidence(`${workflowId}:PAYMENT`)],
        ],
  ...
  private async synchronizeCorporateAction(workflowId: string) {
    ...
    for (const scopeName of ["ISSUANCE", "SECONDARY", "REDEMPTION"])
      await this.run(workflowId, `CORPORATE_PAUSE_${scopeName}`, policy, "pauseScope", () =>
        this.write(policy, policyAbi, "pauseScope", [
          workflow,
          token,
          keccak256(stringToHex(scopeName)),
          evidence(`${workflowId}:CORPORATE_ACTION`),
          evidence(`${workflowId}:PAUSE:${scopeName}`),
        ]),
      );
    ...
    const plan = [
      workflow,
      token,
      BigInt(String(row.numerator)),
      BigInt(String(row.denominator)),
      BigInt(String(row.expected_supply)),
    ] as const;
    await this.run(workflowId, "CORPORATE_RIGHTS_APPROVAL", controller, "approveRightsPlan", () =>
      writeCorporateActionController(this.wallet, {
        controller,
        functionName: "approveRightsPlan",
        args: [...plan, evidence(`${workflowId}:RIGHTS_PLAN`)],
      }),
    );
    await this.run(workflowId, "CORPORATE_AUDIT_APPROVAL", controller, "approveAuditPlan", () =>
      writeCorporateActionController(this.wallet, {
        controller,
        functionName: "approveAuditPlan",
        args: [...plan, evidence(`${workflowId}:AUDIT_PLAN`)],
      }),
    );
    await this.run(workflowId, "CORPORATE_SPLIT_APPLIED", controller, "applySplitBatch", () =>
      writeCorporateActionController(this.wallet, {
        controller,
        functionName: "applySplitBatch",
        args: [workflow, token, accounts.rows.map((item) => item.wallet_address as Address)],
      }),
    );
    const hash = await this.run(
      workflowId,
      "CORPORATE_SPLIT_FINALIZED",
      controller,
      "finalizeSplit",
      () =>
        writeCorporateActionController(this.wallet, {
          controller,
          functionName: "finalizeSplit",
          args: [workflow],
        }),
    );
    await this.pool.query(
    ...
    await this.pool.query(
      "UPDATE workflows SET status='CORPORATE_ACTION_RECONCILED',updated_at=$2 WHERE workflow_id=$1",
      [workflowId, new Date()],
    );
  }
}
