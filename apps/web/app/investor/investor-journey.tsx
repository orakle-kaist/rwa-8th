  ...
  return (
    <main className="investorAppShell">
      <header className="investorTopbar">
        <Link className="investorBrand" href="/">
          K-EQUITY
        </Link>
      ...
      <div className="investorPage">
        <div className="journeyNotice" role="status">
          <span>{message}</span>
          <button type="button" className="textButton" onClick={() => void refresh()}>
            새로고침
          </button>
        </div>
...
function HomeScreen({
  ...
  return (
    <>
      <PageHeading eyebrow="INVESTOR HOME" title="안녕하세요, 모의 투자자님">
        고객이 보는 투자 경험과 기관의 수탁·토큰 처리를 같은 업무로 연결한다.
      </PageHeading>
      <section className="nextTaskCard">
        <div>
          <span>다음 할 일</span>
          <h2>{ready ? "모의 삼성전자 상품 살펴보기" : "계좌 개설 절차 완료하기"}</h2>
          <p>
            {ready
              ? "KRX 1차 주문과 결제완료 권리의 24/7 거래를 구분해 확인한다."
              : "고객확인·위험공시·전용 지갑 상태를 먼저 확인한다."}
          </p>
        </div>
        <Link className="primaryLink" href={withProfile(nextPath, profile)}>
          {ready ? "시장으로 이동" : "온보딩 계속"}
        </Link>
      </section>
      <section className="accountSummary" aria-label="계좌 요약">
        <div>
          <span>사용 가능 USD</span>
          <strong>${money(journey?.primary?.cash?.usdAvailableMinor)}</strong>
        </div>
        <div>
          <span>사용 가능 USDC</span>
          <strong>{money(journey?.primary?.cash?.usdcAvailableMinor, 6)}</strong>
        </div>
        <div>
          <span>결제완료 권리</span>
          <strong>{position?.settledRights ?? "0"}주</strong>
        </div>
        <div>
          <span>진행 중 주문</span>
          <strong>{latestPrimary ? "1건" : "0건"}</strong>
        </div>
      </section>
      <section className="journeyOverview">
        ...
        <p>
          24/7은 KRX가 24시간 거래된다는 뜻이 아니다. 국내 통합 보유총량을 바꾸지 않고 결제완료
          수탁권리를 투자자와 지정 시장조성자 사이에서 재배분한다.
        </p>
      </section>
      <details className="reviewMode">
        <summary>검토 모드</summary>
        <label>
          합성 고객 시나리오
          <select
            value={profile}
        ...
        <div className="buttonRow">
          <Link href="/investor/onboarding">온보딩 다시 시작</Link>
          <Link href={withProfile("/investor/reviewer", profile)}>내부 시험 제어 열기</Link>
        </div>
      </details>
    </>
  );
...
function MarketScreen({
  ...
  profile: DemoProfile;
  demo: Product | undefined;
  candidates: Product[];
}) {
  const [query, setQuery] = useState("");
  const visible = candidates
    .filter((item) =>
      `${item.securityId} ${item.nameKo}`.toLowerCase().includes(query.toLowerCase()),
    )
    .slice(0, 18);
  return (
    <>
      <PageHeading eyebrow="MARKET" title="시장">
        실제 시연 상품과 아직 활성화하지 않은 KOSPI 200 후보를 명확히 구분한다.
      </PageHeading>
      <section className="demoProductCard">
        <div>
          <span className="statePill">PoC 시연 상품</span>
          <h2>{demo?.nameKo ?? "모의 삼성전자 수탁권리"}</h2>
          <p>기초종목 삼성전자(005930) · 1주 = 수탁권리 1단위 = 토큰 1단위</p>
        </div>
        <div className="marketPrice">
          <span>KRX 기준</span>
          <strong>257,000원</strong>
          <small>USD 참고 $186.19</small>
        </div>
        <Link className="primaryLink" href={withProfile("/investor/products/990001", profile)}>
          상품 상세
        </Link>
      </section>
      <section className="candidateSection">
        <div className="sectionTitle">
          <div>
            <h2>KOSPI 200 후보</h2>
            <p>공식 ISIN·수탁지원·판매정책 확인 전에는 주문할 수 없다.</p>
          </div>
          <input
            aria-label="후보 검색"
        ...
        <div className="candidateTable">
          <div className="candidateRow header">
            <span>종목</span>
            <span>구분</span>
            <span>상태</span>
          </div>
          {visible.map((item) => (
            <div className="candidateRow" key={item.securityId}>
              <strong>
                {item.nameKo}
                <small>{item.securityId}</small>
              </strong>
              <span>{item.representative ? "대표 검증종목" : "KOSPI 200"}</span>
              <em>비활성</em>
            </div>
          ))}
        </div>
...
function ProductScreen({
  ...
  return (
    <>
      <PageHeading eyebrow="PRODUCT" title={product?.nameKo ?? "모의 삼성전자 수탁권리"}>
        기초주식을 직접 보유하는 상품이 아니라 인가 해외 증권사가 기록하는 고객별 수탁권리를 제한형
        토큰으로 표시한다.
      </PageHeading>
      <section className="productHero">
        <div className="marketPrice">
          <span>KRX 마지막 기준가격</span>
          <strong>257,000원</strong>
          <small>2026-08-28 · USD/KRW 1,380.3</small>
        </div>
        <div className="quotePair">
          <div>
            <span>지정 MM 매수</span>
            <strong>${money(journey?.normalBidUsdMinor)}</strong>
          </div>
          <div>
            <span>지정 MM 매도</span>
            <strong>${money(journey?.normalAskUsdMinor)}</strong>
          </div>
        </div>
      </section>
      <section className="productActions">
        <article>
          ...
          <h2>KRX 1차 주문</h2>
          <p>
            해외 증권사가 주문을 취합해 국내 증권사에 전달한다. 체결 후 권리와 결제 대기 토큰을 먼저
            만들고 T+2까지 거래를 잠근다.
          </p>
          <Link className="primaryLink" href={withProfile("/investor/orders/new", profile)}>
            1차 주문
          </Link>
        </article>
        <article>
          ...
          <h2>24/7 권리 거래</h2>
          <p>
            이미 국내 결제가 끝난 권리만 지정 시장조성자와 거래한다. 국내 통합계좌 총보유량은 변하지
            않는다.
          </p>
          <Link className="primaryLink" href={withProfile("/investor/secondary", profile)}>
            24/7 거래
          </Link>
        </article>
      </section>
      <section className="disclosureFacts">
        ...
        <dl>
          <div>
            <dt>고객 권리 기준기록</dt>
            <dd>인가 해외 증권사의 고객별 수탁권리 원장</dd>
          </div>
          <div>
            <dt>국내 보유</dt>
            <dd>외국인 통합계좌의 국내 결제완료 총보유량</dd>
          </div>
          <div>
            <dt>토큰</dt>
            <dd>발행·이전·잠금·소각 실행 기록이며 기준장부가 아님</dd>
          </div>
          <div>
            <dt>이전</dt>
            <dd>적격 투자자와 지정 MM 사이의 승인된 정산만 허용</dd>
          </div>
          <div>
            <dt>배당·의결권</dt>
            <dd>기준일 권리 스냅샷과 해외 증권사 승인으로 처리</dd>
          </div>
          <div>
            <dt>환매</dt>
            <dd>KRX 매도와 T+2 후 권리종료·USD 지급·토큰 소각</dd>
          </div>
        </dl>
      </section>
    </>
...
function PrimaryScreen({
  ...
  return (
    <>
      <PageHeading eyebrow="PRIMARY ORDER" title="KRX 1차 주문">
        정수 수량의 KRW 지정가 주문만 지원하며 기초주식 체결 후 수탁권리와 토큰이 생성된다.
      </PageHeading>
      <form action={submit} className="orderTicket">
        <div className="orderInstrument">
          <div>
            <span>모의 삼성전자 수탁권리</span>
            <strong>257,000원</strong>
          </div>
          <small>지정가 · 해당 KRX 영업일 당일 유효</small>
        </div>
        <label>
          주문수량
          <input
        ...
        <label>
          ...
          >
            <option value="USD_LEDGER">USD 고객계좌</option>
        ...
        <div className="orderEstimate">
          ...
          <small>USD/KRW 1,380.3 · 수수료 합성 0 · 미체결 예약 해제</small>
        </div>
        {!confirming ? (
          <button
            type="button"
            disabled={!readiness?.canPlaceNewOrder}
            onClick={() => setConfirming(true)}
          >
            주문 확인
          </button>
        ) : (
          <div className="finalConfirmation">
            <h2>최종 확인</h2>
            <ul>
              <li>부분체결될 수 있다.</li>
              <li>체결 후 권리기입과 T+2 위험 승인이 끝나면 결제 대기 토큰이 발행된다.</li>
              <li>T+2 국내 결제와 수탁 확인 전에는 거래하거나 환매할 수 없다.</li>
              <li>결제불이행 예외처리 책임기관은 인가 해외 증권사다.</li>
            </ul>
            <div className="buttonRow">
              <button type="button" className="textButton" onClick={() => setConfirming(false)}>
                뒤로
              </button>
              <button type="submit">서명하고 주문 제출</button>
            </div>
          </div>
        )}
      </form>
...
function OrderScreen({
  ...
  workflowId: string;
  token: string;
  primary: PrimaryOrder | undefined;
  secondary: SecondaryOrder | undefined;
  redemption: Redemption | undefined;
  openBackstage: () => void;
}) {
  const [timeline, setTimeline] = useState<WorkflowTimeline>();
  useEffect(() => {
    void platformFetch<WorkflowTimeline>(`/workflows/${workflowId}/timeline`, { token })
      .then(setTimeline)
      .catch(() => undefined);
  }, [token, workflowId]);
  ...
  return (
    <>
      <PageHeading eyebrow="ORDER DETAIL" title={`${kind} 상세`}>
        투자자에게 필요한 진행단계만 먼저 보여주고, 기관·장부·체인 증거는 처리 과정 패널에서
        확인한다.
      </PageHeading>
      <section className="orderDetailCard">
        <div className="orderDetailHeader">
          ...
          <span className="statePill">
            {koreanStatus(primary?.status ?? secondary?.status ?? redemption?.status)}
          </span>
        </div>
        <ol className="progressSteps">
          {primarySteps.map((label, index) => (
            <li
              className={
                index < Math.max(1, Math.min(6, timeline?.items.length ?? 1)) ? "complete" : ""
              }
              key={label}
            >
              <span>{index + 1}</span>
        ...
        <div className="orderFacts">
          <div>
            <span>상품</span>
            <strong>모의 삼성전자 수탁권리</strong>
          </div>
          <div>
            <span>요청수량</span>
            <strong>
              {primary?.shareQuantity ??
                secondary?.requestedQuantity ??
                redemption?.requestedQuantity ??
                "-"}
              주
            </strong>
          </div>
          <div>
            <span>배분·체결</span>
            <strong>
              {primary?.allocatedQuantity ??
                secondary?.fillQuantity ??
                redemption?.allocatedQuantity ??
                "-"}
              주
            </strong>
          </div>
          <div>
            <span>잠금·해제</span>
            <strong>{secondary?.cancelledQuantity ?? redemption?.releasedQuantity ?? "-"}주</strong>
          </div>
        </div>
        <button type="button" onClick={openBackstage}>
          처리 과정 보기
        </button>
      </section>
    </>
...
function PositionsScreen({
  ...
  return (
    <>
      <PageHeading eyebrow="PORTFOLIO" title="보유자산">
        거래 가능한 수탁권리와 아직 사용할 수 없는 수량을 섞지 않고 표시한다.
      </PageHeading>
      <section className="holdingCard">
        ...
        <div className="holdingBuckets">
          ...
          <div>
            <span>국내 결제 대기</span>
            <strong>{position?.pendingRights ?? "0"}주</strong>
          </div>
          <div>
            <span>거래·환매 잠금</span>
            <strong>{position?.lockedRights ?? "0"}주</strong>
          </div>
          <div>
            <span>소각 대기</span>
            <strong>{position?.burnPendingTokens ?? "0"}주</strong>
          </div>
          <div>
            <span>USD 지급청구</span>
            <strong>${money(position?.cashClaim?.amountMinor)}</strong>
          </div>
        </div>
        <div className="ledgerComparison">
          <div>
            <span>고객별 수탁권리</span>
            <strong>
              {position
          ...
          <div>
            <span>제한형 토큰 잔액</span>
            <strong>{journey?.redemption?.tokenTotalSupply ?? "-"}단위</strong>
            <small>실행·확인 기록</small>
          </div>
        </div>
        <p className="boundaryCallout">
          토큰은 고객 권리의 기준장부가 아니다. 고객 권리는 인가 해외 증권사의 고객별 수탁권리
          원장에 있고, 토큰은 승인된 발행·이전·잠금·소각을 통제한다.
        </p>
        <div className="buttonRow">
          <Link className="primaryLink" href={withProfile("/investor/secondary", profile)}>
            24/7 거래
          </Link>
          <Link className="subtleLink" href={withProfile("/investor/rights", profile)}>
            환매·권리
          </Link>
        </div>
      </section>
...
function SecondaryScreen({
  ...
  return (
    <>
      <PageHeading eyebrow="CONTROLLED 24/7" title="24/7 제한 거래">
        국내 결제가 완료된 수탁권리를 적격 투자자와 지정 시장조성자 사이에서만 거래한다.
      </PageHeading>
      <div className="secondaryBoundary">
        <strong>KRX 24시간 거래가 아니다.</strong>
        <span>국내 통합계좌 총보유량은 그대로 두고 해외 증권사의 고객별 권리 배분만 바뀐다.</span>
      </div>
      <section className="tradingLayout">
        <div className="quoteBoard">
          <div className="quoteHeader">
            <div>
              <span>지정 시장조성자</span>
              <strong>모의 MM-01</strong>
            </div>
            <small>30초 만료 · 결제완료 재고만 사용</small>
          </div>
          <div className="quoteColumns">
            <div className="bid">
              <span>내가 팔 때</span>
              <strong>${money(journey?.normalBidUsdMinor)}</strong>
              <small>MM 매수</small>
          ...
          <dl>
            <div>
              <dt>가격 기준시각</dt>
              <dd>{journey?.secondary?.informationEffectiveAt ?? "-"}</dd>
            </div>
            <div>
              <dt>오프아워 조정</dt>
              <dd>정상 반쪽 스프레드 0.50%</dd>
            </div>
            <div>
              <dt>거래 가능 보유</dt>
              <dd>{position?.settledRights ?? "0"}주</dd>
            </div>
            <div>
              <dt>거래 제외</dt>
              <dd>결제 대기 {position?.pendingRights ?? "0"}주</dd>
            </div>
          </dl>
        </div>
        <form action={submit} className="orderTicket compact">
          <label>
            지정 MM 호가
            <select
              name="quoteId"
              ...
              required
            >
              <option value="">호가 선택</option>
              {quotes.map((item) => (
                <option key={item.quoteId} value={item.quoteId}>
                  {item.investorSide === "BUY" ? "매수" : "매도"} ·{" "}
                  {item.fundingMode === "USDC_ONCHAIN" ? "USDC" : "USD"} · $
                  {money(item.unitPrice.amountMinor, item.unitPrice.decimals)}
                </option>
              ))}
            </select>
          </label>
          ...
          <div className="orderEstimate">
            <span>체결 방식</span>
            <strong>{quote ? `${quote.remainingQuantity}주까지` : "호가 선택"}</strong>
            <small>부분체결 1회 · 미체결 예약 자동 해제</small>
          </div>
          <button type="submit" disabled={!selected || !readiness?.canPlaceNewOrder}>
            서명하고 주문
          </button>
        </form>
      </section>
...
function ActivitiesScreen({
  ...
  return (
    <>
      <PageHeading eyebrow="ORDERS & ACTIVITIES" title="주문·활동">
        투자자 행동별로 묶고 같은 업무 번호의 기관 처리와 증거를 바로 열 수 있다.
      </PageHeading>
        ...
        {workflows.length ? (
          workflows.map((item) => (
            <article key={item.workflowId}>
              ...
              <div>
                <time>{new Date(item.occurredAt).toLocaleString("ko-KR")}</time>
                <code>{shortId(item.workflowId)}</code>
                <div className="buttonRow">
                  <Link
                    className="subtleLink"
                    href={withProfile(`/investor/orders/${item.workflowId}`, profile)}
                  >
                    상세
                  </Link>
                  <button type="button" onClick={() => openBackstage(item.workflowId)}>
                    처리 과정
                  </button>
                </div>
              </div>
          ...
          <p className="emptyState">아직 주문이나 활동이 없다.</p>
        )}
      </section>
    </>
  );
...
function RightsScreen({
  ...
  journey?: Session["localInvestorJourney"];
  profile: DemoProfile;
  submitRedemption: (data: FormData) => Promise<void>;
  convertDividend: () => Promise<void>;
  submitVote: (value: "FOR" | "AGAINST" | "ABSTAIN") => Promise<void>;
}) {
  const rights = journey?.rights;
  return (
    <>
      <PageHeading eyebrow="RIGHTS & REDEMPTION" title="권리">
        환매, 현금배당, 의결권과 지갑 복구를 각각 독립된 업무로 처리한다.
      </PageHeading>
      <section className="rightsList">
        <article>
          <div>
            <span>환매</span>
            <h2>권리를 종료하고 USD로 받기</h2>
            <p>국내 매도와 T+2 뒤 수탁권리를 지급청구로 바꾸고 토큰을 소각한다.</p>
          </div>
          <form action={submitRedemption} className="inlineAction">
            <input
              name="quantity"
              aria-label="환매 수량"
              type="number"
              ...
              defaultValue="1"
            />
            <button type="submit">환매 요청</button>
          </form>
        </article>
        <article>
          <div>
            <span>현금배당</span>
            <h2>USD {money(rights?.dividend?.netUsdMinor)} 지급</h2>
            <p>
              기준일 수탁권리 스냅샷과 국내 배정총액을 대사한다. USD 지급 후에만 USDC 전환을
              선택한다.
            </p>
          </div>
          <button
            type="button"
            disabled={!rights?.dividend?.quoteId}
        ...
        <article>
          <div>
            <span>의결권</span>
            <h2>{rights?.voting?.titleKo ?? "안건 준비 중"}</h2>
            <p>기준수량 {rights?.voting?.eligibleQuantity ?? "0"}주 · 미응답은 행사하지 않는다.</p>
          </div>
          <div className="buttonRow">
            {(["FOR", "AGAINST", "ABSTAIN"] as const).map((value) => (
              <button key={value} type="button" onClick={() => void submitVote(value)}>
                {value === "FOR" ? "찬성" : value === "AGAINST" ? "반대" : "기권"}
              </button>
            ))}
          </div>
        </article>
        <article>
          <div>
            ...
            <h2>전용 지갑 교체</h2>
            <p>
              기존 지갑 동결 뒤 권리 담당과 준법 담당이 독립 승인한다. 자기보관 USDC는 플랫폼이
              복구하지 못한다.
            </p>
          </div>
          <Link className="subtleLink" href={withProfile("/investor?review=wallet", profile)}>
            복구 상태 보기
          </Link>
        </article>
      </section>
...
function SupportScreen({
  ...
  return (
    <>
      <PageHeading eyebrow="SUPPORT" title="지원">
        플랫폼 기술문의와 증권계좌·거래·규제 민원을 구분해 책임기관에 배정한다.
      </PageHeading>
      <section className="supportLayout">
        <form action={submit} className="orderTicket">
          <label>
            ...
            <select name="type">
              ...
              <option value="ACCOUNT">증권계좌 민원</option>
              <option value="TRADING_ERROR">거래오류 민원</option>
              <option value="REGULATORY">규제·권리 민원</option>
            </select>
          </label>
          <label>
            제목
            <input name="titleKo" required defaultValue="모의 거래 처리 확인 요청" />
          </label>
          <label>
            내용
            <textarea
              name="descriptionKo"
              required
              defaultValue="동일 업무 번호의 처리상태와 책임기관을 확인하고 싶다."
            />
          </label>
        ...
        <div className="complaintList">
          <h2>내 문의</h2>
          {complaints.map((item) => (
            <article key={item.complaintId}>
              <strong>{item.titleKo}</strong>
              <span>{koreanStatus(item.status)}</span>
              <p>{item.responsibleInstitutionId ?? "책임기관 배정 대기"}</p>
            </article>
          ))}
        </div>
      </section>
...
function WorkflowBackstage({
  ...
  return (
    <div className="drawerBackdrop" role="presentation" onMouseDown={close}>
      <aside
        className="backstageDrawer"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header>
          <div>
            <p className="eyebrow">WORKFLOW BACKSTAGE</p>
            <h2 id="backstage-title">처리 과정 보기</h2>
            <code>{workflowId}</code>
          </div>
          <button type="button" className="closeButton" aria-label="닫기" onClick={close}>
            ×
          </button>
        </header>
        <div className="ledgerLegend">
          <span>국내 법적 장부·통합 보유총량</span>
          <span>고객별 수탁권리 원장</span>
          <span>제한형 토큰 장부</span>
          <span>USD·USDC</span>
        </div>
        <p className="boundaryCallout">
          체인 거래가 성공해도 해외 증권사의 권리 원장 반영까지 확인돼야 업무상 완료다.
        </p>
        <div className="laneTimeline">
          {lanes.map((lane) => {
            const items =
                ...
                {items.length ? (
                  items.map((item) => (
                    <article key={item.eventId}>
                      ...
                      <small>
                        {new Date(item.occurredAt).toLocaleString("ko-KR")} · 다음:{" "}
                        {item.nextActionKo}
                      </small>
                      {item.transactionHash ? <code>{shortId(item.transactionHash)}</code> : null}
                    </article>
                  ))
                ) : (
                  <p className="laneEmpty">아직 확인된 단계가 없다.</p>
                )}
              </section>
            );
          })}
        ...
        <Link
          className="primaryLink institutionDeepLink"
          href={`/institution/workflows/${workflowId}`}
        >
          기관 화면에서 같은 업무 보기
        </Link>
      </aside>
